...
from distage_py import ModuleDef, Injector, StandardAxis, Activation, Roots, DIKey


from distage_py.activation import AxisChoiceDef, Axis
...
class Application:
    def __init__(self, greeter: NormalGreeter):  # Note: specifically requires NormalGreeter for demo
...
def main():
    ...
    try:
        injector_everything = Injector(base_module, roots=Roots.everything(), activation=test_activation)
        unused = injector_everything.get(UnusedService)
    ...
    try:
        ...
        app = injector.get(Application)
        unused = injector.get(UnusedService)
